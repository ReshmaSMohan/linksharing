...
public class InvitationController {
    ...
    public ModelAndView sendEmail(HttpSession session, @RequestParam("receiverMail") String email,
        ...
        if (user != null) {
            final String sEmail = "linksharing21@gmail.com";
            final String sPass = "linksharing@1234";
            final String rEmail = email;
            final String subject = "Invitation for Topic" + topicName + "Link Sharing";
            String link = "<html><head></head><body><form name='linkForm' method='get' action='http://localhost:7070/subscribeFromLink'><input type='hidden' name='topicId' value='" + topicId + "'/><input type='hidden' name='userId' value='" + userId + "'/> <button type='submit'>click to subscribe</a> </form></body> </html>";
